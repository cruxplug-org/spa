import {tLangAsync}from'@minejs/server';import {isRTL}from'@minejs/i18n';async function h(t,e="",n="en"){if(!t)return e;if(t.includes(".")&&/^[a-z0-9._]+$/.test(t))try{return await tLangAsync(n,t,void 0)||t}catch{return t}return t}async function m(t,e="",n="en"){return await h(t,e,n)}async function E(t,e="en"){return !t||t.length===0?"":t.map(async r=>await h(r,"",e)).filter(Boolean).join(", ")}async function P(t,e="en"){if(!t)return "Page";let n=await h(t,"",e);try{return v(await n,e)}catch{return n}}async function v(t,e="en"){let n=await tLangAsync(e,"app.name",void 0);return isRTL()?`${n} - ${t}`:`${t} - ${n}`}async function S(t,e,n="en"){let r=t.canonical||`${e.baseUrl}${t.path}`,o=t.robots||e.defaultRobots||"index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1",i=await P(t.title,n),s=await m(t.description,e.defaultDescription||"A modern single-page application",n),a=await E(t.keywords||e.defaultKeywords,n),c=await m(t.expertise,"",n),p=await m(t.experience,"",n),l=await m(t.authority,"",n),u=t.contentType==="article"?"article":"website";return `<!-- \u{1F50D} Core SEO Meta Tags -->
                <meta charset="UTF-8" />
